...
function createEffect(fn, value, options) {
  ...
  const c = createComputation(fn, value, false, STALE),
        s = SuspenseContext && lookup$1(Owner, SuspenseContext.id);
  if (s) c.suspense = s;
  c.user = true;
...
function createResource(pSource, pFetcher, pOptions) {
  ...
  function read() {
    const c = SuspenseContext && lookup$1(Owner, SuspenseContext.id),
          v = value(),
          err = error();
    if (err && !pr) throw err;
...
function useContext(context) {
  let ctx;
  return (ctx = lookup$1(Owner, context.id)) !== undefined ? ctx : context.defaultValue;
}
...
function handleError(err) {
  err = castError(err);
  const fns = ERROR && lookup$1(Owner, ERROR);
  if (!fns) throw err;
...
function assignProp(node, prop, value, prev, isSVG, skipRef) {
  ...
  if (prop === "ref") {
    ...
    if (prop === "class" || prop === "className") className(node, value);else if (isCE && !isProp && !isChildProp) node[toPropertyName$1(prop)] = value;else node[PropAliases[prop] || prop] = value;
  } else {
...
export { $DEVCOMP, $PROXY, $TRACK, DEV, ErrorBoundary, For, Index, Match, Show, Suspense, SuspenseList, Switch, batch, cancelCallback, children, createComponent, createComputed, createContext, createDeferred, createEffect, createMemo, createReaction, createRenderEffect, createResource, createRoot, createSelector, createSignal, createUniqueId, enableExternalSource, enableHydration, enableScheduling, equalFn, from, getListener, getOwner, index as html, indexArray, lazy, mapArray, mergeProps, observable, on, onCleanup, onError, onMount, render, requestCallback, resetErrorBoundaries, runWithOwner, sharedConfig, splitProps, startTransition, untrack, useContext, useTransition };
